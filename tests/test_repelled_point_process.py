...
def test_repelled_point_process(points, core_number, expected):
    "test with point_pattern of 3 simple points and one step in time, with one epsilon"
    d= points.shape[1]
    window = BallWindow(center=[0]*d, radius=5)
    point_pattern = PointPattern(points, window, intensity=1 / np.pi)
    gravity_pp = RepelledPointProcess(point_pattern)
    result = gravity_pp.repelled_point_process(epsilon=1, stop_time=1, core_number=core_number)
    np.testing.assert_array_almost_equal(result, expected)

# test repelled_point_process with p
@pytest.mark.parametrize(
    "points, p, core_number, expected",
    [
        (np.array([[0, 0], [0, -1], [1, 1]]), 3, 1, np.array([[-0.5, 0.5], [-1/5, -12/5], [1.7, 1.9 ]])),
        (np.array([[0, -1], [0, 0], [1, 1]]), 5, 2, np.array([[-0.5, 0.5], [-1/5, -12/5], [1.7, 1.9]])),
...
def test_repelled_point_process_with_p(points, p, core_number, expected):
    "test with point_pattern of 3 simple points and one step in time, with one epsilon"
    d = points.shape[1]
    window = BallWindow(center=[0]*d, radius=10)
    point_pattern = PointPattern(points, window, intensity=1 )
    gravity_pp = RepelledPointProcess(point_pattern)
    result = gravity_pp.repelled_point_process(epsilon=1, stop_time=1, p=p, core_number=core_number)
    np.testing.assert_array_almost_equal(result, expected)
